...
ifeq ($(DEBUG),0)
	MESENFLAGS += -O3
	# Optional: optimize for the local CPU. Only use for builds that won't be
	# distributed (the resulting binary may not run on other machines).
	ifeq ($(NATIVE),true)
		MESENFLAGS += -march=native -mtune=native
	endif
	ifneq ($(LTO),false)
